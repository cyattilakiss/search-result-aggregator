...
class YahooCrawler implements SearchEngineCrawlerInterface
{
    use SearchEngineCrawlerTrait;

    const URL_STRUCTURE = 'https://search.yahoo.com/search?p=%query%&ei=UTF-8&b=%start%&pz=%num%';
    ...
    /**
     * Creates the url for Yahoo search with the query parameters
     *
     * @param string    $query
     * @param int       $num
     * @param int       $page
    ...
    /**
     * Get requested information ['title', 'url'] from a list element of the yahoo search result list
     *
     * @param $item
     * @return array|bool
     */
    private function getResultsFromListElement($item)
    {
        ...
        $node = $crawler->filter('h3 a')->first();
